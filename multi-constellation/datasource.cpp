...
//Load data in the PCD (.pcd) format.
...
//Parse timestamps from the KITTI dataset.
...
//Load data in the original KITTI binary (.bin) format.
...
//The reading method is adaptively selected based on the data format. (To achieve a more compact data footprint, we adopted a custom data format different from KITTI's.）
