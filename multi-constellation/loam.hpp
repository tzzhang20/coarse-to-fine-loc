...
inline cloud_featured<point_type> get_features(point_type *begin, point_type *end)
{
    ...
    {
        point_type *ring = std::stable_partition(ring_start, end, [ring_id](point_type &p)
                                                 { return p.ring == ring_id; });
    ...
    {
        float curv = range[i - 5] + range[i - 4] + range[i - 3] + range[i - 2] + range[i - 1] - range[i] * 10 + range[i + 1] + range[i + 2] + range[i + 3] + range[i + 4] + range[i + 5];
