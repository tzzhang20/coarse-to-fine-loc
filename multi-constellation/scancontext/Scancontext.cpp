...
std::vector<float> eig2stdvec(const Eigen::MatrixXd& _eigmat )
{
...
void SCManager::makeAndSaveScancontextAndKeys(const pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    makeAndSaveScancontextAndKeys( sc );
...
void SCManager::makeAndSaveScancontextAndKeys(const Eigen::MatrixXd& sc )
{
    ...
    polarcontext_tree_built_ = false;

} // SCManager::makeAndSaveScancontextAndKeys

std::pair<int, float> SCManager::detectLoopClosureID (const Eigen::MatrixXd& curr_desc)
{
     ...
     */
    if( polarcontext_invkeys_mat_.size() < NUM_CANDIDATES_FROM_TREE)
    {
    ...
    if(! polarcontext_tree_built_) // to save computation cost
    {
