...
std::pair<double, int> SCManager::distanceBtnScanContext(const Eigen::MatrixXd &_sc1, const Eigen::MatrixXd &_sc2)
{
...
MatrixXd SCManager::makeRingkeyFromScancontext(const Eigen::MatrixXd &_desc)
{
    /*
...
MatrixXd SCManager::makeSectorkeyFromScancontext(const Eigen::MatrixXd &_desc)
{
    /*
...
void SCManager::makeAndSaveScancontextAndKeys(const Eigen::MatrixXd &sc)
{
...
std::pair<int, float> SCManager::detectLoopClosureID(const Eigen::MatrixXd &curr_desc, float *bias)
{
...
size_t SCManager::searchTopN(const Eigen::MatrixXd &query_desc, size_t max_result, search_result *results)
{
...
constexpr const char *scancontext_module_name = "scancontext";

struct scancontext_device : search_device
{
...
static object_id __sc_create_object(search_device *object, const pcl::PointCloud<point_type> &cloud)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return obj_none;
    }

    auto sc_object = static_cast<scancontext_device *>(object);
    auto matrix = sc_object->sc_manager.makeScancontext(cloud);
    sc_object->object_values.push_back(matrix);
    return sc_object->object_values.size() - 1;
}

static size_t __sc_search(search_device *object, object_id searched_target, search_result *results, size_t max_results)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return 0;
    ...
    return sc_object->sc_manager.searchTopN(sc_object->object_values[searched_target], max_results, results);
}

static bool __sc_config(search_device *object, const char *key, const char *value)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return false;
    ...
    {
        sc_object->sc_manager.LIDAR_HEIGHT = std::stod(value);
        return true;
    }
    if (strcmp(key, "num_ring") == 0)
    {
        sc_object->sc_manager.PC_NUM_RING = std::stoi(value);
        sc_object->sc_manager.PC_UNIT_RINGGAP = sc_object->sc_manager.PC_MAX_RADIUS / double(sc_object->sc_manager.PC_NUM_RING);
        return true;
    }
    if (strcmp(key, "num_sector") == 0)
    {
        sc_object->sc_manager.PC_NUM_SECTOR = std::stoi(value);
        sc_object->sc_manager.PC_UNIT_SECTORANGLE = 360.0 / double(sc_object->sc_manager.PC_NUM_SECTOR);
        return true;
    }
    if (strcmp(key, "max_radius") == 0)
    {
        sc_object->sc_manager.PC_MAX_RADIUS = std::stod(value);
        sc_object->sc_manager.PC_UNIT_RINGGAP = sc_object->sc_manager.PC_MAX_RADIUS / double(sc_object->sc_manager.PC_NUM_RING);
        return true;
    }
    ...
    {
        sc_object->sc_manager.NUM_CANDIDATES_FROM_TREE = std::stof(value);
        return true;
    }

    if (strcmp(key, "ratio") == 0)
    {
        sc_object->sc_manager.SEARCH_RATIO = std::stod(value);
        return true;
    }
...
static void __sc_join(search_device *object, object_id id)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return;
    ...
    sc_object->sc_manager.makeAndSaveScancontextAndKeys(sc_object->object_values[id]);
}

static void __sc_join_flush(search_device *object)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return;
    ...
    auto sc_object = static_cast<scancontext_device *>(object);
    sc_object->sc_manager.buildIndex();
}

static ssize_t __sc_serialize(search_device *object, FILE *fp, object_id id)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return -1;
...
static ssize_t __sc_deserialize(search_device *object, FILE *fp, object_id &id)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return -1;
...
static bool __sc_save(search_device *object, object_id id, const char *filename)
{
    if (object == nullptr || strcmp(object->module->name, scancontext_module_name) != 0)
    {
        return false;
    ...
    Eigen::MatrixXd &M = sc_object->object_values[id];
...
static search_device *__sc_create();
static void __sc_destroy(search_device *object);

static search_module __sc_module = {
    .name = scancontext_module_name,
    .create_object = &__sc_create_object,
    .search = &__sc_search,
    .config = &__sc_config,
    .join = &__sc_join,
    .join_flush = &__sc_join_flush,

    .serialize = &__sc_serialize,
    .deserialize = &__sc_deserialize,
    .save = &__sc_save,

    .create = &__sc_create,
    .destroy = &__sc_destroy,
};

static search_device *__sc_create()
{
    auto object = new scancontext_device;
    object->module = &__sc_module;
    return object;
}
...
static void __sc_destroy(search_device *object)
{
    ...
    delete static_cast<scancontext_device *>(object);
}

register_search_module(__sc_module);
